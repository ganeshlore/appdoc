...
function treeMaker(tree, params) {
    ...
    let fileName = treeParamas[Object.keys(tree)[0]] !== undefined && treeParamas[Object.keys(tree)[0]].fileName !== undefined ? treeParamas[Object.keys(tree)[0]].fileName : Object.keys(tree)[0].fileName;
    let path = treeParamas[Object.keys(tree)[0]] !== undefined && treeParamas[Object.keys(tree)[0]].path !== undefined ? treeParamas[Object.keys(tree)[0]].path : Object.keys(tree)[0].path;
    card.innerHTML = '<p class="tree__container__step__card__p" id="card_' + Object.keys(tree)[0] + '" filename="'+fileName+'" filepath="'+path+'">' + fileName + '</p>';
    ...
    for (let i = 0; allCards.length > i; i++) {
        allCards[i].addEventListener('click', function (event) {
            params.card_click(event.target, allCards[i].getAttribute("filepath"));
        })
    }
...
function iterate(tree, start = false, from = '') {
    ...
    for (const key in tree) {
        ...
        let textCard = treeParamas[key] !== undefined && treeParamas[key].fileName !== undefined ? treeParamas[key].fileName : key;
        let path = treeParamas[key] !== undefined && treeParamas[key].path !== undefined ? treeParamas[key].path : key;

        treeContainer.innerHTML += '<div class="tree__container__step"><div class="tree__container__step__card" id="' + key + '"><p id="card_' + key + '" class="tree__container__step__card__p" filename="'+textCard+'" filepath="'+path+'">' + textCard + '</p></div></div>';
